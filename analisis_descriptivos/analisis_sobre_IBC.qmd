...
nombres_tam_empresa <- c("1" = "Independiente", "2" = "Pequeña", "3" = "Micro", "4" = "Mediana", "5" = "Grande", "6" = "Super", "Desconocido" = "Desconocido")
...
```{r grafico_ibc_poblacion_general, echo=FALSE}
...
```{r grafico_ibc_por_genero, echo=FALSE}
...
names_estadisticas_usar_grafico = nombres_estadisticas[names(nombres_estadisticas) %in% estadisticas_usar_grafico]
colores_usar_grafico =  colores_genero[names(colores_genero) %in% gnero_usar_grafico]
...
graf_est_ibc_serie_historica_tam_empresa <- 
  ...
  mutate(categoria_tamano_empresa_general = factor(categoria_tamano_empresa_general, levels = c("1", "2", "3", "4", "5", "6", "Desconocido"),ordered = T)) %>% 
    ...
    geom_line(linewidth = 1) +
  geom_smooth(method = 'loess', formula = 'y ~ x', linetype = 3, alpha = 0.5,se = F) + 
